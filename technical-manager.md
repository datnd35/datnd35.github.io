...
<div class="lt-hero">
  <h1>Learning Tracks: Technical Manager</h1>
  <p>Kiến thức và kỹ năng cho Technical Manager — từ nền tảng kỹ thuật đến quản lý con người, thời gian và tổ chức team hiệu quả.</p>
</div>

{% assign all_posts = site["technical-manager"] | sort: 'date' | reverse %}
{% assign tech_posts = all_posts | where: "track", "tech-knowledge" %}
{% assign mgmt_posts = all_posts | where: "track", "management-skills" %}

<div class="lt-section">
  <h2>🧑‍💼 Technical Manager Tracks</h2>
  <ul class="lt-track-list">

    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('tm-tech', this)">
        Technical Knowledge (JS, Microservices, Auth, AI) <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="tm-tech">
        <ul>
    ...
    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('tm-mgmt', this)">
        Management Skills (Time, Architecture, PM Interview) <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="tm-mgmt">
        <ul>
          {% for post in mgmt_posts %}
          <li>
            <span class="post-date">{{ post.date | date: "%Y-%m-%d" }}</span>
            <a href="{{ post.url }}">{{ post.title }}</a>
          </li>
