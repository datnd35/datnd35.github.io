...
<div class="lt-hero">
  ...
  <p>Kiến thức thực chiến về tối ưu hiệu năng — từ Frontend Performance, Web Vitals, Rendering Optimization đến Backend Profiling, Caching và Database Query Tuning.</p>
</div>

{% assign all_posts = site.performance | sort: 'date' | reverse %}
{% assign frontend_posts  = all_posts | where: "track", "frontend-performance" %}
{% assign webvitals_posts = all_posts | where: "track", "web-vitals" %}
{% assign rendering_posts = all_posts | where: "track", "rendering-optimization" %}
{% assign backend_posts   = all_posts | where: "track", "backend-performance" %}
{% assign caching_posts   = all_posts | where: "track", "caching-strategy" %}
{% assign db_posts        = all_posts | where: "track", "database-optimization" %}

<div class="lt-section">
  <h2>1. Frontend Performance</h2>
  <ul class="lt-track-list">

    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('perf-frontend', this)">
        Frontend Performance Fundamentals <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="perf-frontend">
        <ul>
    ...
    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('perf-webvitals', this)">
        Core Web Vitals (LCP, FID, CLS, INP) <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="perf-webvitals">
        <ul>
          {% for post in webvitals_posts %}
          <li>
            <span class="post-date">{{ post.date | date: "%Y-%m-%d" }}</span>
            <a href="{{ post.url }}">{{ post.title }}</a>
          </li>
    ...
    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('perf-rendering', this)">
        Rendering Optimization (Lazy Load, Code Splitting, SSR) <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="perf-rendering">
        <ul>
          {% for post in rendering_posts %}
          <li>
            <span class="post-date">{{ post.date | date: "%Y-%m-%d" }}</span>
            <a href="{{ post.url }}">{{ post.title }}</a>
          </li>
...
<div class="lt-section">
  <h2>2. Backend &amp; Infrastructure Performance</h2>
  <ul class="lt-track-list">

    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('perf-backend', this)">
        Backend Profiling &amp; Optimization <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="perf-backend">
        <ul>
    ...
    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('perf-caching', this)">
        Caching Strategy (Redis, CDN, HTTP Cache) <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="perf-caching">
        <ul>
          {% for post in caching_posts %}
          <li>
            <span class="post-date">{{ post.date | date: "%Y-%m-%d" }}</span>
            <a href="{{ post.url }}">{{ post.title }}</a>
          </li>
    ...
    <li class="lt-track-item">
      <button class="lt-track-btn" onclick="togglePanel('perf-db', this)">
        Database Query Optimization <span class="arrow">▶</span>
      </button>
      <div class="lt-panel" id="perf-db">
        <ul>
